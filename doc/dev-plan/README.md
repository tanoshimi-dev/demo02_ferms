...
- 機能実装とあわせて、対応する **テストコード** を追加する
- テストは **必ず対象サービスを起動した状態で実施** し、実行確認までを完了条件に含める
...
- frontend / backend / db のうち、今回の変更対象サービスは必ず起動してからテストする
- API テストや画面テストは、対象サービスが疎通可能な状態で実行する
- backend の単体テストだけで完了扱いにせず、必要な結合確認も行う
- 認証変更では portal 連携前提の確認、予約変更では DB を含む確認を行う
- テストコードを追加した場合は、対応する実行手順も `dev-plan` と実装に合わせて維持する
