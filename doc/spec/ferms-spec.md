...
1. 利用者はメールアドレスとパスワード、または OAuth 2.0 により認証できる。
...
| 認証 | JWT + OAuth 2.0 | 認証処理は NestJS 側に集約する |
...
- 認可が必要な API は JWT ベースで保護する。
- 管理者専用 API は role ベースで制御する。

## 12. 認証・認可要件

1. パスワードは平文保存しない。
2. パスワードハッシュには `bcrypt` を使用する。
3. JWT secret はソースコードに保存しない。
4. JWT や OAuth 2.0 の認証情報はサーバー側設定で管理する。
5. 管理者専用機能は一般利用者から分離する。
